...
`pnpm build` matters here: chapters are prerendered at build time with
`generateStaticParams`, and `dynamicParams` is disabled, so a chapter registered in
`chapters.ts` without a matching file (or the reverse) is caught by the build, not at runtime.
