...
`src/shared/lib/api-client.ts` is the single place a network request is made. It exports:
...
Every feature has a `*.keys.ts` file exporting hierarchical key factories. This is what makes
...
when a component unmounts, and every service supports it.
...
const profile = await getUserById(userId, context);
```
...
`/onboarding`, and `/dashboard`. It checks only for the **presence** of a cookie starting with
`better-auth.session`. No token validation, no API call. Unauthenticated visitors are sent to
...
- `:root { … }` holds the raw values in `oklch()`, including the brand ramp
  `--ost-blue-one` → `--ost-blue-four` and semantic tokens like `--destructive` and `--success`
