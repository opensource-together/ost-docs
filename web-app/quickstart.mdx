...
| `NEXT_PUBLIC_FRONTEND_URL` | The app's own origin; used by better-auth and mock CORS | Injected by `dev:mock` |
...
<AccordionGroup>
  ...
  <Accordion title="Tailwind v4: there is no tailwind.config">
    ...
    design tokens to utilities, and `:root` holds the raw `oklch()` values, including the
    brand ramp `--ost-blue-one` … `--ost-blue-four`.
